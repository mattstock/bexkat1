...
set_multicycle_path -from * -to [get_registers {*bexkat1|ccr[*]}] -setup -start 2
set_multicycle_path -from * -to [get_registers {*bexkat1|ccr[*]}] -hold -start 1
...
create_generated_clock -name led_clk -master_clock clock_50 -source clock_50 -divide_by 4 [get_registers {*led_matrix:led_matrix0|state.STATE_CLOCK} ]
create_generated_clock -name led_clk_pin -source [get_registers {*led_matrix:led_matrix0|state.STATE_CLOCK} ] [get_ports {rgb_clk}]
